...
if [[ -n "$container" ]]; then
    ...
    cmd="docker exec -i $HAS_TTY --workdir /work $container zsh"
    log_success "[+] $cmd"
    $cmd
    exit 0
fi
...
if [[ -n "${LIMIT_MEMORY_PERCENT:-}" ]]; then
    total_ram=$(free -m | grep "Mem:.*" | xargs | cut -d ' ' -f 2)
    log_success "[+] Total amount of installed RAM is $total_ram MiB"
    ram_limit=$(echo "$total_ram * ($LIMIT_MEMORY_PERCENT/100.0)" | bc -l | cut -d '.' -f 1)
    ram_limit=${ram_limit/./}
    cmd+=" --memory=${ram_limit}m"
fi
